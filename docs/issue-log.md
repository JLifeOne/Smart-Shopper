...
### 2026-01-13 19:10 UTC — Supabase function deploy failed on Deno lockfile version
Summary: `supabase functions deploy` failed with `Unsupported lockfile version '5'` when bundling Edge Functions.
Impact: `menus-policy` and `menus-llm` could not deploy, blocking rollout.
Root Cause: `supabase/functions/deno.lock` was generated by Deno 2 (lockfile v5), but Supabase Edge bundler uses Deno 1 and only supports lockfile v4.
Fix: Temporarily switch to Deno 1.46.3, regenerate `supabase/functions/deno.lock` (v4), deploy functions, then switch back to Deno 2.6.4. The lockfile now reports `"version": "3"` and deploy succeeds.
Validation: `supabase functions deploy menus-policy` and `supabase functions deploy menus-llm` completed successfully after lockfile regeneration.
Prevention: Regenerate `supabase/functions/deno.lock` with Deno 1 before Edge deployments, and avoid committing Deno 2 lockfiles when functions need bundling.

### 2026-01-30 15:00 UTC — End-to-end validation skipped, partial fixes regressed flows
Summary: A bugfix was shipped without reading `docs/issue-log.md` or verifying the full flow against the last known good commit, causing a repeat of a prior Metro resolution failure and new menu save regressions.
Impact: Lost engineering time, duplicate incidents, and user-visible failures in menu save flows.
Root Cause: Debugging stopped after a narrow symptom fix; no holistic validation (bundle → client load → feature flow), no git history trace to the last good state, and no checklist tie-back to prior issues.
Fix: Add explicit guardrails in `AGENTS.md` for holistic E2E validation (issue-log review + git history trace) and require documenting results in this log.
Validation: Reproduced the failure on the broken commit, bisected to the last good commit, applied the holistic validation checklist (Metro bundle, dev client load, menu save end-to-end), and confirmed all steps pass.
Prevention: Treat end-to-end validation as mandatory for all debugging/fixes; always read `docs/issue-log.md`, identify the last known good commit, validate the full user journey, and record outcomes here.
