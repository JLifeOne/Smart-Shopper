...
| `feature_promo_notifications` | `false` | Promo alerts | Enable to show the promo inbox and push toggles in Account → Promo alerts. |
...
Note: Push provider selection is controlled by `EXPO_PUBLIC_NOTIFICATIONS_PROVIDER` (`expo` | `onesignal` | `auto`).
