...
async function fetchRuntimeConfig(client: SupabaseClient<any>): Promise<RuntimeFlags> {
  const query = client
    ...
    .select('value')
    .eq('key', 'brand_insights');

  const { data, error } = await (query as any).maybeSingle?.() as
    | { data: RuntimeConfigRow | null; error: null }
  ...
  const brandEnabled = parseBrandInsights((data as RuntimeConfigRow | null | undefined) ?? undefined);
  flags = { ...flags, brandInsights: brandEnabled };
